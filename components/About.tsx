...
export default function LandingPage() {
  ...
  className="mx-auto mb-24 p-10 rounded-3xl bg-white dark:bg-gray-900 shadow-lg border max-w-5xl grid grid-cols-2 sm:grid-cols-4 gap-8"
>
  {[
    ...
    <motion.div
      ...
      key={index}
      className="group relative text-center transition cursor-pointer rounded-2xl overflow-hidden p-6"
    >
      {/* Light gradient overlay on hover */}
      <div className="
        absolute inset-0 opacity-0 group-hover:opacity-40
        bg-gradient-to-br from-[#F54E02]/20 to-transparent
      ...
      <div className="relative z-10">
        ...
        <div className="text-gray-500 dark:text-gray-300 mt-1 text-sm font-medium">
          {stat.label}
        </div>
      </div>
    </motion.div>
      ...
      <section className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-14 items-center mt-10">
        <motion.div
          ...
          transition={{ duration: 0.7 }}
          className="rounded-3xl overflow-hidden shadow-xl hover:scale-[1.02] transition"
        >
  ...
  <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 max-w-4xl mx-auto mt-14">
    {[
      ...
      <motion.div
        key={index}
        whileHover={{ scale: 1.12, y: -4 }}
        className="group relative flex flex-col items-center p-6 rounded-2xl border shadow-sm bg-white dark:bg-gray-900 transition cursor-pointer overflow-hidden"
      >
        {/* Light gradient overlay on hover */}
        <div className="
          absolute inset-0 opacity-0 group-hover:opacity-50
          bg-gradient-to-br from-[#F54E02]/10 to-transparent
          transition-opacity duration-300 pointer-events-none
        "></div>
        ...
        {/* Label */}
        <p className="mt-3 font-semibold relative z-10">{item.label}</p>
      </motion.div>
    ))}
  </div>
</section>
